...
% $Id: pcaklm.m,v 1.11 2008/07/31 11:19:27 davidt Exp $
...
function [w,truefrac] = pcaklm (type,a,frac)
	...
	elseif (frac >= 1)			% Return FRAC dimensions.
		n = abs(frac); if (n > k), error('illegal dimensionality requested'); end
		I = I(1:n); sv = sum(v); 
		if (sv ~= 0), truefrac = cumsum(v(1:n))/sv; else, truefrac = 0; end;
