...
% It is saved in the directory NAME and B refers to it. If desired the
% data and target fields are compressed (after appropriate rescaling)
% to NBITS unsigned integers. The stored datafile can be retrieved by
% 
%    B = DATAFILE(NAME)
%
% B is a 'mature' datafile, i.e. a dataset distributed over a number of
% files. This has only advantages over a 'raw' datafile defined for a 
% directory of images in case of substantial pre- and postprocessing, due
% to the overhead of the dataset construct of B.
% Default FILESIZE = 1000000.
%
...
function c = savedatafile(dfile_in,featsize,dirr,nbits,filesize)
  ...
  if isdatafile(b) & ~iscell(dfile_in)
	...
	for j=1:nfiles             % run over all files to be created
		...
		if ~isempty(featsize)
			if prod(featsize) ~= getfeatsize(a)
